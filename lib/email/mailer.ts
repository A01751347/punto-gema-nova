...
export async function sendOrderConfirmationEmail(
    ...
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #463931; background-color: #f5f4ed; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 40px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 15px rgba(0,0,0,0.1);">
        ...
        <div style="padding: 30px;">
          ...
          <h2 style="color: #1a1a1a; font-size: 20px; margin-top: 30px; margin-bottom: 15px;">Resumen del Pedido</h2>
          
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            <thead>
              <tr style="background-color: #f5f4ed;">
                <th style="padding: 10px; text-align: left; border-bottom: 2px solid #1a1a1a;">Producto</th>
                <th style="padding: 10px; text-align: center; border-bottom: 2px solid #1a1a1a;">Cantidad</th>
                <th style="padding: 10px; text-align: right; border-bottom: 2px solid #1a1a1a;">Precio</th>
              </tr>
            </thead>
            <tbody>
              ${itemsHtml}
          ...
          <div style="text-align: right; margin-top: 20px; padding-top: 20px; border-top: 2px solid #eee;">
            ...
            <p style="margin: 10px 0 0 0; font-size: 20px; color: #1a1a1a;"><strong>Total:</strong> $${orderDetails.total.toFixed(2)}</p>
          </div>
          
          <div style="margin-top: 40px; padding: 20px; background-color: #f5f4ed; border-radius: 8px;">
            <h3 style="color: #1a1a1a; margin-top: 0;">¿Qué sigue?</h3>
          ...
          <div style="margin-top: 30px; text-align: center;">
            <a href="${process.env.NEXT_PUBLIC_APP_URL}/cuenta/pedidos" style="display: inline-block; background-color: #1a1a1a; color: #ffffff; padding: 12px 30px; text-decoration: none; border-radius: 4px; font-weight: 600;">Ver mi pedido</a>
          </div>
        </div>
        
        <div style="background-color: #f5f4ed; padding: 20px; text-align: center; font-size: 14px; color: #6b6358;">
          <p style="margin: 5px 0;">Punto Gema Nova — Joyería Artesanal Elegante</p>
  ...
  `;
    ...
    await sendEmail({
        to: email,
        subject: `Confirmación de Pedido #${orderNumber} - Punto Gema Nova`,
        html,
    });
}
...
export async function sendNewsletterWelcomeEmail(email: string, firstName?: string): Promise<void> {
    ...
    <head>
      <meta charset="utf-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Bienvenido a Punto Gema Nova</title>
    </head>
    <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #463931; background-color: #f5f4ed; margin: 0; padding: 0;">
      <div style="max-width: 600px; margin: 40px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 15px rgba(0,0,0,0.1);">
        <div style="background-color: #1a1a1a; color: #ffffff; padding: 30px; text-align: center;">
          <h1 style="margin: 0; font-size: 28px;">¡Bienvenido a Punto Gema Nova!</h1>
        </div>
        ...
        <div style="padding: 30px;">
          ...
          <ul style="font-size: 16px; margin-bottom: 30px; padding-left: 20px;">
            <li style="margin-bottom: 10px;">Acceso anticipado a nuevas colecciones</li>
            <li style="margin-bottom: 10px;">Conoce nuestros materiales y piedras</li>
          ...
          <div style="margin-top: 30px; text-align: center;">
            <a href="${process.env.NEXT_PUBLIC_APP_URL}/quiz" style="display: inline-block; background-color: #1a1a1a; color: #ffffff; padding: 12px 30px; text-decoration: none; border-radius: 4px; font-weight: 600;">Descubre tu estilo ideal</a>
          </div>
        </div>
  ...
  `;
    ...
    await sendEmail({
        to: email,
        subject: '¡Bienvenido a Punto Gema Nova!',
        html,
    });
}
